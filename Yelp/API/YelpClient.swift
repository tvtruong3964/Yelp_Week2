...
class YelpClient: BDBOAuth1RequestOperationManager {
    ...
    func search(with term: String, completion: @escaping ([Business]?, Error?) -> ()) -> AFHTTPRequestOperation {
        return search(with: term, sort: nil, distance: nil, categories: nil, deals: nil, offset: nil, completion: completion)
    }

    func search(with term: String, sort: YelpSortMode?, distance: Int?, categories: [String]?, deals: Bool?, offset: Int?,completion: @escaping ([Business]?, Error?) -> ()) -> AFHTTPRequestOperation {
